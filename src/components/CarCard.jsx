...
const CarCard = ({ car }) => {
  return (
    ...
    >
      {/* Image Container - White Background for Studio Look */}
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-xl md:text-2xl font-serif font-bold text-warmDark leading-tight">{car.name}</h3>
        ...
        {/* Specs Grid */}
        <div className="grid grid-cols-3 gap-2 mb-6 border-t border-warmDark/10 pt-4">
          <div className="flex flex-col items-center text-center">
            <FaUser className="text-gold mb-1" size={14} />
            <span className="text-sm font-sans text-warmDark/80">{car.specs.passengers} Seats</span>
          </div>
          <div className="flex flex-col items-center text-center">
            <FaCogs className="text-gold mb-1" size={14} />
            <span className="text-sm font-sans text-warmDark/80">{car.specs.transmission}</span>
          </div>
          <div className="flex flex-col items-center text-center">
            <FaGasPump className="text-gold mb-1" size={14} />
            <span className="text-sm font-sans text-warmDark/80">{car.specs.fuel}</span>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-warmDark text-warmLight py-3 rounded-xl font-bold tracking-wide hover:bg-gold hover:text-warmDark transition-all duration-300 shadow-md hover:shadow-lg"
        >
          <FaWhatsapp size={20} />
          Book Now
        </button>
